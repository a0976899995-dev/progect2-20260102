...
function renderPage() {
    ...
    const textSegments = data.dialogue; 
    ...
    if (subStep < textSegments.length - 1) {
        ...
        if (nextIndicator) nextIndicator.classList.remove("hidden");
        ...
        if (nextIndicator) nextIndicator.classList.add("hidden");
...
function saveGame() {
    playSFX(sounds.click);
    const gameState = { currentStep, subStep, conflictPoints, commPoints, impressPoints };
...
function showFinalEnding() {
    ...
    let title = "", content = "", finalSound = sounds.endNormal;
